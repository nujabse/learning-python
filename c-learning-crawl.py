...
class Crawler(object):
    ...
    def __init__(self, name, start_url):
        ...
        self.domain = 'http://akaedu.github.io/book/'
...
class CScraper(Crawler):
    """
    Linux C编程一站式学习
    源地址 https://akaedu.github.io/book/
    """
    def parse_menu(self, response):
        soup = BeautifulSoup(response.content, "html.parser")
        menu_tag = soup.find_all(class_ = "toc")[0]
        for link in menu_tag.find_all('a'):
            url = link.get("href")
            print(url)
            if not url.startswith("http"):
                url = self.domain + url # 补全为全路径
            yield url

    def parse_body(self, response):
        try:
            soup = BeautifulSoup(response.content, 'html.parser')
            classes = ["preface", "toc", "sect1", "chapter", "appendix","bibliography", "index"]
            for i in classes:
...
if __name__ == '__main__':
    ...
    # crawler.run()
    start_url = "http://akaedu.github.io/book/"
    crawler = CScraper("Linux C编程一站式学习", start_url)
    crawler.run()
